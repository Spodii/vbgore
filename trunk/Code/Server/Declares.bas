...
'************            Official Release: Version 1.0.12           ************
'************                 http://www.vbgore.com                 ************
'*******************************************************************************
'*******************************************************************************
...
'Records the server's FPS, number of users connect and the number of NPCs made every second
'View the data made by running ToolServerFPSViewer
Public Const DEBUG_MapFPS As Boolean = True

'Use the logging system, which makes detailed outputs of tons of stuff the server does. Priceless               '//\\LOGLINE//\\
'for debugging, but can slow down the server quite a bit. Not recommended for any public heavily-tested         '//\\LOGLINE//\\
'server. Because having the logging even in the engine slows it down (even if the logging is disable),          '//\\LOGLINE//\\
'although very insignificantly, the ToolLogRemover has been added which will delete any line in the             '//\\LOGLINE//\\
'server that has the LogLine marker in it                                                                       '//\\LOGLINE//\\
Public Const DEBUG_UseLogging As Boolean = True                                                                 '//\\LOGLINE//\\

'Records the number of packets (by packet ID) going in to / out from the server
Public Const DEBUG_RecordPacketsOut As Boolean = True
Public Const DEBUG_RecordPacketsIn As Boolean = True

'********** Public CONSTANTS ***********

'Change to 1 to enable database optimization on runtime. Can take a little bit of time on large databases, but
'may increase performance of the database access.
Public Const OptimizeDatabase As Byte = 0

'If we run the server in high priority, making the computer focus on the server application more than any
'other. Recommended to leave on. If you ever experience problems with your computer slowing down to a near-freezing
'state when the server is running (especially in the IDE), set the value to 0.
Public Const RunHighPriority As Byte = 1

'Screen resolution (must be identical to the values on the client!) - used for distance calculations so the server
'knows what the client can see
...
'How long an object's data (ObjData values) can remain in memory unused
Public Const ObjMemoryLife As Long = 300000 '5 minutes

'How long the maps last in memory when no users are on it. Keep in mind when a map is removed from memory, any NPC
'and object on it is deleted. This means when the user goes back to the map, NPCs will be alive and in their starting
'position, along with all the ground objects will be deleted. If you have "boss spawns" or anything similar that have
'a very long spawn time, it is recommended you raise this value to accommodate for that. Unfortunately, this will also
'mean the server will use more RAM holding unused maps.
Public Const EmptyMapLife As Long = 180000  '3 minutes

'Amount of time that must elapse for certain user events (in miliseconds) - used to prevent spamming / flooding
...
'Change this value to add a cost to sending mail (0 = no cost)
...
'Maximum amount of ground objects allowed on a single map tile
...
'Stat constants
Public Const STAT_ATTACKWAIT As Long = 1000 'How many milliseconds a user has to wait until he can attack again

'How many quests a user can accept at once
Public Const MaxQuests As Byte = 8

'Time that must elapse for NPC to make another action (in milliseconds) after attacking
...
Public Const Group_InviteTime As Long = 10000   '(DO NOT raise this value above 255 seconds!) The time (in milliseconds) in which the user has to accept a group invitation
...
End Type
Type PacketCache
    'Used for the packet caching system that is used to reduce bandwidth by figuring out if any values in packets
    'have changed from the last time the packet was sent. Currently, only the Server_MakeChar packet makes use
    'of this system.
...
'States if the server is unloading
...
Public Const MaxNPCs As Integer = 32765 'This is a little less then the max of an integer, just in case
...
'Used to track the number of packet (by their IDs) that have come in / gone out from the server
