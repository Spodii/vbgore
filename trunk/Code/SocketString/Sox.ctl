    ...
    Else
        ...
        Else
            ...
            Case 8209   'byte array - Optimal Data Type
                Let tmpData = inData
            Case 8      'string, if we recieve a string, its assumed we are linemode (What the hell does this mean ???)
                Let tmpData = StrConv(inData, vbFromUnicode)
            Case Else
                Let tmpData = StrConv(CStr(inData), vbFromUnicode)
            End Select
            If UBound(tmpData) = soxERROR Then ' A value of -1 is returned from UBound if there was no data
                Call RaiseError(soxERROR, 0, Procedure, "Invalid Data. Possible cause: No Data to send eg. a blank string")
                Let SendData = soxERROR
            Else
                Call Long2Byte2(UBound(tmpData), tmpSize) ' I use UBound here instead of UBound + 1 to test the buffer on the other side EXACTLY !
                ReDim Preserve Send(inSox).Buffer(Send(inSox).Size + 4 + UBound(tmpData) + 1) As Byte ' 4 = Sized, UBound + 1 = DataLength
                Call apiCopyMemory(Send(inSox).Buffer(Send(inSox).Size + 1), tmpSize(0), 4)
                Let Send(inSox).Size = Send(inSox).Size + 4
                Call apiCopyMemory(Send(inSox).Buffer(Send(inSox).Size + 1), tmpData(0), UBound(tmpData) + 1)
                Let Send(inSox).Size = Send(inSox).Size + UBound(tmpData) + 1
                If apiWSAAsyncSelect(Sockets(inSox).Socket, UserControl.hWnd, ByVal Sockets(inSox).uMsg, ByVal FD_CLOSE Or FD_READ Or FD_WRITE) = SOCKET_ERROR Then
                    Call RaiseError(inSox, Err.LastDllError, Procedure, "On apiWSAAsyncSelect")
                End If
            End If
...
':) Ulli's VB Code Formatter V2.19.5 (2006-Jul-31 18:20)  Decl: 412  Code: 1064  Total: 1476 Lines
':) CommentOnly: 102 (6.9%)  Commented: 383 (25.9%)  Empty: 112 (7.6%)  Max Logic Depth: 9
