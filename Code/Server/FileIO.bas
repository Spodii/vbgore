...
'More info: http://www.vbgore.com/GameServer.FileIO.Log                                                             '//\\LOGLINE//\\
'*****************************************************************                                                  '//\\LOGLINE//\\
                                                                                                                    '//\\LOGLINE//\\
...
'*****************************************************************
'Loads and returns the data of mail from the database specified by the mail's index
'More info: http://www.vbgore.com/GameServer.FileIO.Load_Mail
'*****************************************************************
...
Dim i As Long
    ...
    'Make sure the database returned some values
    If Not DB_RS.EOF Then
        ...
        'Check for a valid object string - if ObjStr has a value, then we must split up the string
        'to find the ObjIndex and Amount for all the objects in the mail
        If LenB(ObjStr) Then
...
'More info: http://www.vbgore.com/GameServer.FileIO.Load_Maps_Temp
'*****************************************************************
...
Dim i As Long
    ...
    'Don't load an already loaded map
    If MapInfo(MapNum).DataLoaded = 1 Then Exit Sub
    
    'Set the map's data as loaded
    ...
    'Open the map's temp file
    FileNum = FreeFile
    Open ServerTempPath & "m" & MapNum & ".temp" For Binary Access Read As #FileNum
    
        'Get the NPC information
        'Start by getting the number of NPCs
        Get #FileNum, , intNumNPCs
        
        'If theres any NPCs, resize the NPCInfo() array accordingly then grab all the NPC information
        'directly into the NPCInfo() array in one Get call - keep in mind this just grabs the values, we
        'still have to load each NPC individually so the server recognizes them properly
        If intNumNPCs > 0 Then
            ReDim NPCInfo(1 To intNumNPCs) As NPCLoadData
            Get #FileNum, , NPCInfo()
        End If

        'Load all the tile information directly into the Data() array
    ...
    If intNumNPCs > 0 Then
        For i = 1 To intNumNPCs
            ...
            'Load the NPC with Load_NPC, and get the NPCIndex
...
'More info: http://www.vbgore.com/GameServer.FileIO.Save_Maps_Temp
'*****************************************************************
Dim NPCInfo() As NPCLoadData
Dim intNumNPCs As Integer
Dim FileNum As Byte
...
Dim Y As Long
    ...
    Open ServerTempPath & "m" & MapNum & ".temp" For Binary Access Write As #FileNum
        ...
        'Store the number of NPCs first, and if theres 1 or more NPCs, store the NPC info
...
'*****************************************************************
'Loads the Data() of a map (holds the tile data) - only called at runtime
'to create the .temp copy of maps
'More info: http://www.vbgore.com/GameServer.FileIO.Load_Maps_Data
'*****************************************************************
...
'*****************************************************************
'Unloads the map data from memory, and any NPCs and ground objects on it
'More info: http://www.vbgore.com/GameServer.FileIO.Unload_Map
'*****************************************************************
Dim i As Long

    'Make sure the data is even loaded so we don't try unloading an unloaded map
    If MapInfo(MapNum).DataLoaded = 1 Then
    
        'If the UnloadTimer = 0, its time must have not been set, so set the map to be
        'alive for EmptyMapLife milliseconds
        If MapInfo(MapNum).UnloadTimer = 0 Then
            MapInfo(MapNum).UnloadTimer = EmptyMapLife + timeGetTime
            Exit Sub
        End If
        
        'Check if enough time has elapsed to remove the map
        If MapInfo(MapNum).UnloadTimer + EmptyMapLife < timeGetTime Then
            ...
            'Completely unload the map's data from memory
...
'*****************************************************************
'Loads all of the map's .inf, .dat and .map files and creates the .temp files
'More info: http://www.vbgore.com/GameServer.FileIO.Load_Maps
'*****************************************************************
Dim LoopC As Long
Dim Map As Long
    ...
    'Retrieve the number of maps total
    NumMaps = Val(Var_Get(DataPath & "Map.dat", "INIT", "NumMaps"))
    ReDim MapInfo(1 To NumMaps)

    'Create the MapUsers array for each map, which is an array of the users (by index) on the map
    ...
    'Loop through every map, load the data and create the .temp files
...
'*****************************************************************
'Loads the NPC information from the database then saves it in .temp
'files for fast access when the NPC needs to be loaded
'More info: http://www.vbgore.com/GameServer.FileIO.Save_NPCs_Temp
'*****************************************************************
...
Dim j As Byte
    ...
    'Resize the NPC array to fit the one NPC we are using (this is just temporary, and the NPCList array isn't used yet)
...
'*****************************************************************
'Loads the names of NPCs used in quests so we don't have to load them into
'memory just to get their name for a quest
'More info: http://www.vbgore.com/GameServer.FileIO.Load_NPC_Names
'*****************************************************************
Dim i As Long
 
    'Grab all of the indexes of NPCs used in quests in descending order (ordered by the index of the quest's NPC)
    ...
    For i = 1 To UBound(NPCName)
        ...
        If NPCName(i) = "." Then
            ...
            'Grab the NPC's name from the database
...
'*****************************************************************
'Loads a NPC from their .temp file and returns its index (NPCIndex)
'More info: http://www.vbgore.com/GameServer.FileIO.Load_NPC
'*****************************************************************
Dim ObjNums As NPCBytes
Dim NPCIndex As Integer
Dim FileNum As Byte
    ...
    'If there is not enough room in the NPCList() array, resize it to fit the new NPC
    'LastNPC, not to be confused with the index of the last NPC loaded, holds the highest NPC index used (in other words, the
    'UBound() of the NPCList() array)
    ...
    With NPCList(NPCIndex)
        ...
        Open ServerTempPath & "n" & NPCNumber & ".temp" For Binary Access Read As #FileNum
            ...
            'Get the number of objects the NPC has (both Vend and Drop)
            Get #FileNum, , ObjNums
            
            'Create the arrays for the drop objects if needed
            ...
            'Create the array for the vending objects if needed
            ...
            'Get the NPC's information (pretty much everything but the object information)
            Get #FileNum, , NPCList(NPCIndex)
        
            'Set the NPC's thralled value. If the NPC was not thralled, the RespawnCounter will be overwritten
            'later when the NPC dies and ignored when the NPC is alive. If the NPC is thralled, the RespawnCounter
            'will be used to determine how long the NPC has left to live.
...
'*****************************************************************
'Loads the data of all the objects from the database and creates the
'.temp files for them for fast access later when they need to be loaded
'More info: http://www.vbgore.com/GameServer.FileIO.Load_OBJs
'*****************************************************************
...
Dim FileNum As Byte
    ...
    'Get the highest object index (Sort by id, descending, only get 1 entry, only return id)
...
'*****************************************************************
'Loads all of the quest information from the database and saves it as .temp
'files for fast database-free access later
'More info: http://www.vbgore.com/GameServer.FileIO.Load_Quests
'*****************************************************************
    ...
    'Resize the quests array to fit all of the quests
    ReDim QuestData(1 To NumQuests)

    'Retrieve the data from the database (grab all information from every quest)
    DB_RS.Open "SELECT * FROM quests", DB_Conn, adOpenStatic, adLockOptimistic
    
    'Grab the information from the database and put it into the appropriate QuestData() variable
    Do While Not DB_RS.EOF  'Loop until we reach the end of the recordset
...
'*****************************************************************
'Loads the Server.ini file which contains some server settings
'More information can be found in the Server.ini file: \ServerData\Server.ini
'More info: http://www.vbgore.com/GameServer.FileIO.Load_ServerIni
'*****************************************************************
Dim TempSplit() As String
Dim ts() As String
Dim i As Byte
...
Dim j As Long
    ...
    'Get the total number of servers (for multi-server support)
...
'*****************************************************************
'Loads information on a user and puts it into the appropriate index in the
'UserList() array based off of the UserIndex argument. Used when users
'log in to the server.
'More info: http://www.vbgore.com/GameServer.FileIO.Load_User
'*****************************************************************
...
'*****************************************************************
'Writes the mail information to the database with values specified by the MailData
'argument, into the MailIndex index
'More info: http://www.vbgore.com/GameServer.FileIO.Save_Mail
'*****************************************************************
Dim s As String
Dim i As Long
...
'More info: http://www.vbgore.com/GameServer.FileIO.Save_User
'*****************************************************************
Dim BankStr As String
Dim InvStr As String
Dim MailStr As String
...
Dim i As Long
    ...
    'On special occasions, we want to skip the typical saving routine and save before the user is even disconnected
    'In other words, if DoNotSave is set to 1, then do not save ;)
...
'*****************************************************************
'Save the inbound (client -> server) packet information to \Logs\ServerID\packetsin.txt
'More info: http://www.vbgore.com/GameServer.FileIO.Save_PacketsIn
'*****************************************************************
Dim LoopC As Long
Dim FileNum As Byte
Dim s As String
    
    'Build the string holding all the values
    For LoopC = 0 To 254
        s = s & LoopC & ": " & DebugPacketsIn(LoopC) & vbNewLine
    Next LoopC
    s = s & "255: " & DebugPacketsIn(LoopC)
    
    'Make sure the directory exists
    MakeSureDirectoryPathExists LogPath
    
    'Delete the old file if it exists
    If Server_FileExist(LogPath & ServerID & "\packetsin.txt", vbNormal) Then Kill LogPath & ServerID & "\packetsin.txt"
...
'*****************************************************************
'Save the outbound (server -> client) packet information to \Logs\ServerID\packetsin.txt
'More info: http://www.vbgore.com/GameServer.FileIO.Save_PacketsOut
'*****************************************************************
Dim LoopC As Long
Dim FileNum As Byte
Dim s As String
    ...
    Next LoopC
    s = s & "255: " & DebugPacketsOut(LoopC)
    
    'Make sure the directory exists
    MakeSureDirectoryPathExists LogPath
    
    'Delete the old file if it exists
    If Server_FileExist(LogPath & ServerID & "\packetsout.txt", vbNormal) Then Kill LogPath & ServerID & "\packetsout.txt"
...
'More info: http://www.vbgore.com/GameServer.FileIO.Save_FPS
'*****************************************************************
Dim FileNum As Byte
...
'*****************************************************************
'Gets a string from a text file (often with the suffix .ini)
'More info: http://www.vbgore.com/GameServer.FileIO.Var_Get
'*****************************************************************

    'We don't want to log EVERY Var_Get call, since that will get very messy, so give the option to not log '//\\LOGLINE//\\
    If DontLog = 0 Then Log "Call Var_Get(" & File & "," & Main & "," & Var & ")", CodeTracker              '//\\LOGLINE//\\
    
    'Make 1000 spaces for the buffer (this means the maximum line size is 1000)
    Var_Get = Space$(1000)
    
    'Grab the value from the file
    GetPrivateProfileString Main, Var, vbNullString, Var_Get, 1000, File
    
    'Trim off the extra spaces
    Var_Get = RTrim$(Var_Get)
    
    'If the string has a value, there will be an extra character on the far right side of the string so crop it off
...
'More info: http://www.vbgore.com/GameServer.FileIO.Var_Write
'*****************************************************************
